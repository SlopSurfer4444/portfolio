...
Ограничения задаются несколькими слоями:

- Фиксированная рабочая папка и профиль файлового доступа, закрывающий пути авторизации; общая оболочка отключена в конфигурации модельной сессии.
- Зарегистрированные хостом инструменты с проверкой аргументов и привязкой к разрешённым чатам.
...
Отдельная исследовательская ветка содержит три собственных Windows-native Rust-прототипа: запуск и наблюдение процессов, Job Objects, restricted token и low integrity, запуск в приостановленном состоянии, проверки дескрипторов и путей. Они отделены от действующего Python/Node/WSL-пути.
